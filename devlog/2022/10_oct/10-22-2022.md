...
God... the backwards tipping too triggers me

Okay I'm gonna get something going, no IMU unfortunately due to that "sometimes works" part, need to think about that

Have `activeScan` as a way to know current sweep plane

3:40 PM back from a break, feel spent already

Going to check the scan output per sweep and make sure that if it's open space it goes as far as it can... more than 2 forward gaits

Doing a scan on flat ground

- u2
  - smallest value is 29.92
- u1
  - smallest value is 28.82
- m
  - smallest value is 27.80
- d1
  - 18.23
- d2
  - 6.97
...
In this current scan values are:
...
Need to design and print a quick little battery clip to prevent it from falling out.

Thankfully with the existing design it should be pretty easy.

6:03 PM

Yeah it is important to scan the sky, since it can pull the board off from the IMU

Yeah need that IMU check/pings web interface "help I'm stuck"